...
void MMU_SetCTRL (int ctrl) {
	...
	int firstValidAddress;
	registerCTRL_MMU=ctrl&0x3;
	if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) {
		firstValidAddress = 0;
		firstInvalidAddress=MAINMEMORYSIZE; // Protected (OS) mode
	} else {
		firstValidAddress = registerBase_MMU;
		firstInvalidAddress=registerBase_MMU+registerLimit_MMU; // Non-protected (USER) mode
		registerMAR_MMU+=registerBase_MMU;
	}
	switch (registerCTRL_MMU) {
		case CTRLREAD:
		case CTRLWRITE:
			if (registerMAR_MMU < firstInvalidAddress && firstValidAddress <= registerMAR_MMU) {
