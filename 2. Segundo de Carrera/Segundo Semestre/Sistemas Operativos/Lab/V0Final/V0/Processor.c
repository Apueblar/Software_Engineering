...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		// Instruction ADD: acc = op1 + op2
		case ADD_INST:
			registerAccumulator_CPU= operand1 + operand2;
			registerPC_CPU++;
			break;
		
		// Instruction SHIFT (SAL and SAR): acc = acc/(2)^op1
		case SHIFT_INST: 
		...
		// Instruction JUMP: PC += op1
		case JUMP_INST: 
			registerPC_CPU+= operand1;
			break;
			  
		// Instruction ZJUMP: if(acc=0) PC+=op1
		case ZJUMP_INST: 
		...
		// Instruction WRITE: WRITE in op1 the Value Acc
		case WRITE_INST: 
		...
		// Instruction READ: READ from op1 the Value to Acc
		case READ_INST: 
		...
		// Instruction INC: acc += 1
		case INC_INST: 
			registerAccumulator_CPU+= operand1;
			registerPC_CPU++;
			break;
		
		// Instruction HALT: Finish/Stop executing
		case HALT_INST: 
			registerPSW_CPU=POWEROFF;
			break;

		// Instruction MEMADD: acc = op1 + read(op2) //op2 is an address
		case MEMADD_INST:
			// READ THE ADDRESS of OPERAND2
			// Tell the main memory controller from where
			...
			// Add the operand 1 to the accumulator
