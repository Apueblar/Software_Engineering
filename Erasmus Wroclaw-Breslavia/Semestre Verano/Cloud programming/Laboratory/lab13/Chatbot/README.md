...
## 8. Lambda notification from chat messages

The chat application now includes an AWS Lambda notification flow required for the latest assignment.

### Trigger word

The selected trigger word is:

```text
lambda
```

When this word appears anywhere in a chat message, the frontend sends the complete message to AWS Lambda. The normal chat behaviour is preserved: the message is still stored and displayed through the existing backend.

### Files changed

- `frontend/src/routes/+page.svelte` — checks the message before sending it and invokes Lambda when the trigger word is present.
- `lambda/message_notifier/lambda_function.py` — receives the message payload and publishes an SNS notification.
- `main.tf` — creates the Lambda function, publishes it, exposes a Lambda Function URL, creates the SNS topic/subscription and passes the Function URL to the frontend task.

### Terraform resources added

```text
aws_lambda_function.chat_message_notifier
aws_lambda_function_url.chat_message_notifier_url
aws_sns_topic.chat_message_notifications
aws_sns_topic_subscription.chat_message_notifications_email
data.archive_file.chat_message_notifier_zip
```

The Lambda function receives the SNS topic ARN through the `SNS_TOPIC_ARN` environment variable. The frontend receives the Lambda endpoint through `PUBLIC_LAMBDA_FUNCTION_URL` and the trigger word through `PUBLIC_LAMBDA_KEYWORD`.

After `terraform apply`, the SNS e-mail subscription must be confirmed from the configured inbox before notifications can be delivered.
