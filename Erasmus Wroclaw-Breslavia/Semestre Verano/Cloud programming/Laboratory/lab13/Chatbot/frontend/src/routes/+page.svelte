<script lang="ts">
	...
	const lambdaKeyword = (env.PUBLIC_LAMBDA_KEYWORD ?? 'lambda').toLowerCase();
	const lambdaFunctionUrl = env.PUBLIC_LAMBDA_FUNCTION_URL ?? '';
	...
	const notifyLambdaIfNeeded = async (message: string) => {
		if (!lambdaFunctionUrl || !message.toLowerCase().includes(lambdaKeyword)) return;

		try {
			await fetch(lambdaFunctionUrl, {
				method: 'POST',
	...
	const sendMessage = async () => {
		...
		try {
			await notifyLambdaIfNeeded(messageToSend);
