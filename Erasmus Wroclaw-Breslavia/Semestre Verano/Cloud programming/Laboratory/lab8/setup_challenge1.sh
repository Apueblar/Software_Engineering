...
GITHUB_TOKEN=$(aws secretsmanager get-secret-value \
  --region us-east-1 \
  --secret-id "myproject/github_token" \
  --query "SecretString" \
  --output text)

su - ubuntu -c "cd ; git clone https://pwr-cloudprogramming:${GITHUB_TOKEN}@github.com/pwr-cloudprogramming/clprog2026-a04-thu1304.git token_clone/"