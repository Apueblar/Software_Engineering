...
variable "alert_email" {
  ...
  type        = string
  default     = "293867@student.pwr.edu.pl"
}
...
output "backend_url" {
  ...
  value       = "http://${aws_lb.app_lb.dns_name}:5000/chat/all?username=health"
}
...
resource "aws_sns_topic_policy" "ecs_alerts_policy" {
...
resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  ...
  alarm_description   = "Alarm when ECS CPU utilization exceeds threshold"
  alarm_actions       = [aws_sns_topic.ecs_alerts.arn]

  dimensions = {
    ClusterName = aws_ecs_cluster.ecs_chatbot.name
  }
}

resource "aws_cloudwatch_event_rule" "ecs_task_stopped" {
  name        = "ecs-task-stopped-rule"
  description = "Triggered when any ECS task in the account is stopped"
